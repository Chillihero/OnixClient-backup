...
function keyboard(key, isDown)
    ...
    --FUNCTIONAL KEYS
    if key == 0x6D and isDown == true then client.execute("execute /hub") end --/hub
    if key == x and isDown == true then client.execute("execute /connection") end --/connection (to check your connected server)

    --SKYWARS
    if key == 0x61 and isDown == true then client.execute("execute /q sky") end --queue Sky Solo
    if key == 0x62 and isDown == true then client.execute("execute /q sky-duos") end --queue Sky Duo
    if key == x and isDown == true then client.execute("execute /q sky-trios") end --queue Sky Trio
    if key == 0x63 and isDown == true then client.execute("execute /q sky-squads") end --queue Sky Squad
